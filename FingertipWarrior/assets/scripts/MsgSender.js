var mod_GameMsgProtocol = require("./lib/Gamemsg");
var mod_MsgRecognizer = require("./MsgRecognizer");
var mod_Protobuf = protobuf;
/**
 * WebSocket
 * 
 * @var {WebSocket}
...
function __makeUint8Array(oMsg) {
    ...
    console.log(strMsgCode);
    let nMsgCode = mod_GameMsgProtocol.GameMsg.MsgCode[strMsgCode];
    console.log(nMsgCode);
    // 获取消息类型
    ...
    // 获取对应的 Protobuf 消息
    let oProtobufMsg = mod_GameMsgProtocol.GameMsg[strMsgType];
    if (!oProtobufMsg) {
        // 如果对应的 Protobuf 消息为空,
...
function __makeMsgObj(oUint8Array) {
    ...
    let strMsgCode = mod_GameMsgProtocol.GameMsg.MsgCode[nMsgCode];
